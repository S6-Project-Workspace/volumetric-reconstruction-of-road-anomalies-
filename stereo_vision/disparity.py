...
class WLSFilter:
    ...
    def filter_disparity(
        ...
        # Prepare right disparity and create appropriate WLS filter
        if disparity_right is not None:
            ...
            # Use matcher-based filter with left-right confidence
            ...
            # Use generic filter (no LRC confidence) for edge-aware smoothing only
            wls_filter = cv2.ximgproc.createDisparityWLSFilterGeneric(use_confidence=False)
            wls_filter.setLambda(self.lambda_val)
            wls_filter.setSigmaColor(self.sigma_color)
