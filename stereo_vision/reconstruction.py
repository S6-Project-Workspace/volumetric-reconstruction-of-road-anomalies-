...
class OutlierRemover:
    ...
    def _compute_mean_neighbor_distances(self, xyz: np.ndarray) -> np.ndarray:
        ...
        Uses scipy KDTree for efficient O(n log n) nearest neighbor queries.
        ...
        # Query k+1 nearest neighbors (including self at distance 0)
        k = min(self.k_neighbors + 1, n_points)
        distances, _ = tree.query(xyz, k=k)
        
        # Exclude self-distance (column 0) and compute mean of k nearest
        if k > 1:
            mean_distances = np.mean(distances[:, 1:], axis=1).astype(np.float32)
        else:
